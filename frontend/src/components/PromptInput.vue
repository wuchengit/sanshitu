<template>
  <div class="bg-[#1a1a20] border border-[#2a2a32] rounded-lg p-3">
    ...
    <div class="mt-2 text-[10px] text-[#999] font-mono whitespace-pre-wrap max-h-[100px] overflow-y-auto bg-[#0a0a0e] rounded p-1.5">{{ text || '选择结构和风格后自动生成提示词' }}</div>
    <div class="flex gap-1 mt-1.5 flex-wrap items-center">
      ...
      <button @click="doTranslate" :disabled="translating" class="px-2 py-1 bg-[#25252d] border border-[#333] rounded text-[10px] text-[#aaa] hover:bg-[#333]">{{ translating ? '⏳' : '🌐 翻译' }}</button>
      <select v-model="lang" class="bg-[#25252d] border border-[#333] rounded px-2 py-1 text-[10px] text-[#aaa]">
        <option value="zh">🇨🇳 中文</option><option value="en">🇬🇧 English</option><option value="ja">🇯🇵 日本語</option><option value="ko">🇰🇷 한국어</option><option value="fr">🇫🇷 Français</option><option value="es">🇪🇸 Español</option>
      </select>
      ...
      <button @click="$emit('gen')" :disabled="loading" class="ml-auto px-4 py-2 bg-gradient-to-r from-[#667eea] to-[#764ba2] rounded-md text-white text-xs font-semibold disabled:opacity-40 hover:opacity-90">
        {{ loading ? '⏳ 生成中...' : '🎨 生成图片' }}
      </button>
    </div>
  </div>
